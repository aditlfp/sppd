<nav x-data="{ open: false }"">
    ...
    <div id="fullscreen-menu"
        ...
        <div id="divIsi"
            ...
            <a href="{{ route('dashboard') }}" class="text-white font-bold text-xl flex items-center justify-center gap-2 btn bg-sky-500 w-full rounded-sm p-2 px-[25svw] text-center"><i class="ri-home-9-line"></i>Dashboard</a>

            <a href="{{ route('main_sppds.index') }}" class="text-white font-bold text-xl flex items-center justify-center gap-2 btn bg-sky-500 w-full rounded-sm p-2 text-center"><i class="ri-file-paper-2-fill"></i> SPPD</a>

            @if (in_array( auth()->user()->name, ['SULASNI', 'PARNO', 'DIREKTUR', 'DIREKTUR UTAMA', 'admin']))
                <a href="{{ route('page-verify-sppd')}}" class="text-white font-bold text-xl flex items-center justify-center gap-2 btn bg-sky-500 w-full rounded-sm p-2  text-center">
                    <i class="ri-file-check-line"></i>Verifikasi SPPD
                </a>
            @endif
            @if (auth()->user()->role_id == 2)
                <a href="{{ route('eslons.index') }}" class="text-white font-bold text-xl flex items-center justify-center gap-2 btn bg-sky-500 w-full rounded-sm p-2 text-center"><svg
                ...
                <a href="{{ route('pocket_moneys.index') }}"
                    class="text-white font-bold text-xl flex items-center justify-center gap-2 btn bg-sky-500  w-full rounded-sm p-2 text-center"><svg xmlns="http://www.w3.org/2000/svg"
            ...
            <form method="POST" action="{{ route('logout') }}" class="w-full mt-[20%]">
                @csrf
                <button type="submit" class="text-white font-bold text-xl flex items-center justify-center gap-2  w-full rounded-sm p-2 text-center btn bg-red-500"><svg
                        xmlns="http://www.w3.org/2000/svg" width="20" viewBox="0 0 24 24" fill="currentColor">
                        <path
